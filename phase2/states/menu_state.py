...
class MenuState(BaseState):
    def __init__(self, **kwargs):
        ...
        self.button_dict = {}
        self.button_hover = [False for _ in range(3)]
    ...
    def get_event(self, event):
        ...
        if len(self.button_dict) != 0:
            for i in range(3):
                self.button_hover[i] = False
                if self.button_dict[i].collidepoint(self.mouse_pos):
    ...
    def draw(self, screen):
        ...
        for i, option in enumerate(self._options):
            ...
            button_text = self.medium_font.render(option, True, (pygame.Color('black') if not self.button_hover[i] else pygame.Color('magenta')))
